...
<body>
	...
	//	if(!("WebSocket" in window)){
			// no sockets - better set up Flash :-O
	...
	//	sop.start();
		...
		// setInterval(function(){sop.stroke.apply(sop)}, Math.floor(Math.random()*5000) + 5000);
		// 	setInterval(function(){sop.poke.apply(sop)}, Math.floor(Math.random()*5000) + 5000);
		// 	
...
/*	function send(){
	...
	function startBroadcastingID() { 
		...
		setTimeout(function() { broadcastIntervalID = setInterval(loop,1);reset(); }, Math.floor(Math.random()*1000)) ; 
		changeColour("ffffff");
